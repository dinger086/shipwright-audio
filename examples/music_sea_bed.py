...
def sea_bed():
    ...
    pad = Track(
        ...
        compose.progression(["Dm", "Bb", "F", "C"], bpm, beats_per_chord=4, repeats=2, timing="beats"),
        gain_db=-6,
    )
    lead = Track(
        instruments.saw_lead(cutoff=1400),
        compose.melody([74, 77, 76, 74, 72, 74, None, 69, 72, 74],
                       bpm, note_beats=[2,1,1,2,1,1,2,2,2,4], start_beat=4, timing="beats"),
        gain_db=-11,
    )
    bass = Track(
        instruments.sub_bass(),
        compose.bassline(["D", "Bb", "F", "C"], bpm, octave=2, beats_per_note=4, repeats=2, timing="beats"),
        gain_db=-7,
    )
    return RenderSpec(tracks=[pad, lead, bass], tempo=bpm, time_unit="beats", time_signature=(4, 4),
